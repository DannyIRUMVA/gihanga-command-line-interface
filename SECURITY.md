...
Gihanga CLI and also where the boundaries are.

In general Gihanga CLI is a coding agent that runs locally within the security boundary
...
Gihanga treats the local user account and files writable by that account as inside
the same trust boundary as the Gihanga process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Gihanga configuration, they can generally influence Gihanga or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Gihanga grants that write access or crosses
an operating-system privilege boundary.

Gihanga relies on users installing trustworthy extensions and loading trustworthy
ubumenyi and only to use Gihanga within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in Gihanga or another package in
...
- Local code execution or sandboxing behavior (Gihanga intentionally does not have a sandbox)
- Behavior of Gihanga extensions or ubumenyi installed by the user
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.gihanga`,
  `~/.gihanga/agent/models.json`, workspace files, `AGENTS.md`, ubumenyi, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Gihanga itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against Gihanga.
...
For example, a report showing that malicious contents written to a trusted Gihanga
configuration file cause Gihanga to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Gihanga.
