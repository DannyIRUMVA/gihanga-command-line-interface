...
describe("theme syntax highlighting", () => {
	...
	it("colors diff additions and deletions in fenced diff blocks", () => {
		...
		expect(lines[0]).toBe("\x1b[38;2;229;72;77m-old\x1b[39m");
		expect(lines[1]).toBe("\x1b[38;2;32;96;61m+new\x1b[39m");
	});
	...
	it("keeps cli-highlight default styled scopes mapped to theme styles", () => {
		expect(highlightCode("const re = /foo+/gi;", "javascript")[0]).toContain(
			"\x1b[38;2;229;190;1m/foo+/gi\x1b[39m",
		);
		expect(highlightCode("@decorator", "python")[0]).toBe("\x1b[38;2;138;166;178m@decorator\x1b[39m");
		expect(highlightCode("<div></div>", "html")[0]).toContain("\x1b[38;2;0;161;222mdiv\x1b[39m");
	});
});
