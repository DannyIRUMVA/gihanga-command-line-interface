...
// ============================================================================
// OAuth API key resolution from the user agent auth.json
// ============================================================================
...
/**
 * Resolve API key for a provider from the user agent auth.json
 *
...
/**
 * Check if a provider has credentials in the user agent auth.json
 */
...
/** Path to the real agent config directory */
export const PI_AGENT_DIR = join(homedir(), CONFIG_DIR_NAME, "agent");

/**
 * Get an AuthStorage instance backed by the user agent auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `gihanga-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
