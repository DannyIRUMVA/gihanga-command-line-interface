...
if [ "$(uname -s 2>/dev/null || true)" = "Darwin" ]; then
	echo "macOS: open a new Terminal tab or run: hash -r"
	echo 'If gihanga is still not found, run: export PATH="$HOME/.local/bin:$PATH"'
	echo 'To make that permanent, add the export line above to ~/.zshrc.'
fi
`;
    ...
    <section data-reveal id="install" class="mx-auto w-[90%] max-w-[1200px] py-8">
      ...
      <div class="grid gap-5 lg:grid-cols-2">
        <article class="mobile-pad rounded-2xl border border-cyan-400/20 bg-gihanga-glass p-6 backdrop-blur">
          ...
          <div class="mt-4 rounded-xl border border-cyan-300/20 bg-cyan-400/10 p-4 text-sm text-[#dce1fb]"><p class="font-bold text-gihanga-text">macOS Terminal step</p><p class="mt-1">After install, open a new Terminal tab or run <code class="rounded bg-white/10 px-1.5 py-0.5 font-mono text-gihanga-emerald">hash -r</code>. If macOS still says <code class="rounded bg-white/10 px-1.5 py-0.5 font-mono text-gihanga-emerald">gihanga: command not found</code>, run <code class="rounded bg-white/10 px-1.5 py-0.5 font-mono text-gihanga-emerald">export PATH=&quot;$HOME/.local/bin:$PATH&quot;</code> and add that line to <code class="rounded bg-white/10 px-1.5 py-0.5 font-mono text-gihanga-emerald">~/.zshrc</code> to keep it permanent.</p></div>
        </article>
